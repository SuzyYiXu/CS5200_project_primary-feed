    ...
    <div className="flex h-screen bg-gray-50">
      ...
      <div className="w-64 bg-slate-900 text-white flex flex-col">
        ...
        <nav className="flex-1 p-4 space-y-2">
          ...
          <a href="/operations" className="block p-3 hover:bg-slate-800 rounded-md">Operations</a>
          <a href="#" className="block p-3 bg-blue-600 rounded-md">Community</a>
      ...
      <div className="flex-1 overflow-auto p-8">
        <header className="mb-6 flex justify-between items-end">
          <div>
            ...
            <p className="text-gray-500 text-sm">Manage users, donors, and beneficiaries</p>
          </div>
          {/* Action button representing an INSERT operation */}
          ...
          {['volunteers', 'donors', 'beneficiaries', 'staff'].map((view) => (
            <button
              ...
              className={`pb-3 capitalize transition-all ${
                activeView === view ? 'text-blue-600 border-b-2 border-blue-600' : 'text-gray-400 hover:text-gray-600'
              }`}
            >
              {view}
            </button>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
          <table className="w-full text-left text-sm">
            ...
            <tbody className="divide-y divide-gray-100">
              {/* Placeholder Rows representing your DML samples */}
              {activeView === 'volunteers' ? (
                <tr>
        ...
        {activeView === 'volunteers' && (
          <div className="mt-8 bg-amber-50 border border-amber-200 p-4 rounded-lg">
            <h4 className="text-amber-800 font-bold text-xs uppercase mb-1">Database Logic Check</h4>
            <p className="text-amber-700 text-xs leading-relaxed">
              When assigning shifts, the <strong>trg_volunteer_shift_no_overlap_insert</strong> trigger 
              is active. The system will reject any shift entry that overlaps with an existing time block 
              for the same Volunteer ID.
            </p>
          </div>
        )}
      </div>
